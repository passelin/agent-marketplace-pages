...
async function generateData() {
    ...
    JSON.stringify(
      {
        ...
        counts: {
          ...
          agents: 0, instructions: 0, skills: 0, hooks: 0, workflows: 0,
          total: items.length, local: localCount, external: externalCount, remote: remoteCount,
        },
      },
      null,
      2
  ...
  // Stub files for resource types not generated by this pipeline
  const emptyList = (filters) => JSON.stringify({ items: [], filters }, null, 2);
  for (const [name, filters] of [
    ["agents.json",       { models: [], tools: [] }],
    ["instructions.json", { extensions: [] }],
    ["skills.json",       { sourceTypes: [], categories: [] }],
    ["hooks.json",        { events: [], tags: [] }],
    ["workflows.json",    { triggers: [] }],
  ]) {
    const dest = path.join(outputDir, name);
    if (!fs.existsSync(dest)) fs.writeFileSync(dest, emptyList(filters));
  }
